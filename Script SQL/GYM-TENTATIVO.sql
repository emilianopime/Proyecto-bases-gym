...
COMMENT ON COLUMN Clientes.Genero IS 'M: Masculino, F: Femenino, O: Otro/Prefiero no decirlo';
...
COMMENT ON COLUMN Membresias.AccesoPiscina IS '0: No tiene acceso, 1: Sí tiene acceso';
COMMENT ON COLUMN Membresias.AccesoClases IS '0: No tiene acceso, 1: Sí tiene acceso';
...
COMMENT ON COLUMN ClientesMembresias.Estado IS 'Estado actual de la membresía del cliente (Activa, Vencida, Congelada, Cancelada, Pendiente de pago)';
...
-- 6. Tabla Clases (Definición de las clases ofrecidas)
CREATE TABLE Clases (
...
COMMENT ON COLUMN Clases.DiaSemana IS 'Día en que se imparte la clase. "Variable" si no tiene un día fijo.';
...
COMMENT ON COLUMN AsistenciaClases.HoraCheckIn IS 'Hora exacta del check-in del cliente a la clase, formato HH:MM. Opcional.';
